    ...
    return useMutation({
        ...
        mutationFn: async (): Promise<MarkAsReadResponse> => {
            const response = await api.post<MarkAsReadResponse>(
                `/user/notifications/read/unread-count`
            );
        ...
        onSuccess: async (_data) => {
            try {
                ...
                queries.forEach(([queryKey, cached]) => {
                    ...
                    if (changed) {
                        // Reset unread_count safely
                        next.unread_count = 0;
                        if (next.meta) {
                            next.meta.unread_count = 0;
                        }

                        if (!Array.isArray(next.data) && next.data && typeof next.data === 'object') {
                            next.data.unread_count = 0;
                            if (next.data.meta) {
                                next.data.meta.unread_count = 0;
                            }
                        }
        ...
        onError: (error) => {
            const fallback = "Failed to mark notification as unread.";
